-- The same save, but it can be told what it expected to find.
--
-- Two people with the same dish open — or one person with two tabs, which is
-- commoner — both pressed Save and the second silently replaced the first.
-- Every line, with no record that anything had been lost.
--
-- `p_expect` is the `updated_at` the screen loaded. When it is given and the
-- row has moved on since, nothing is written and the function raises, so the
-- action can say who to reload rather than quietly discarding somebody's work.
-- Omitted, it behaves exactly as before, which is what every other caller
-- (import, the paste, a new dish) wants.
...
begin
  if p_expect is not null then
    ...
    if found_at is not null and found_at <> p_expect then
      raise exception 'stale_recipe' using errcode = '40001';
    end if;
  end if;
