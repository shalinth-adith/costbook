-- The version guard, answering in a way that reaches the caller.
--
-- Migration 22 raised its refusal with SQLSTATE 40001, "serialization
-- failure". That is a class of error the connection pooler is entitled to
-- assume is transient, so it retried the call until the gateway gave up: a
-- refusal that should be instant took sixty seconds and arrived as a timeout,
-- while an ordinary save went through in one. Everything else here is
-- identical to 22.

create or replace function save_recipes(
  p_recipes jsonb,
  p_lines jsonb,
  p_with_meta boolean default true,
  p_expect timestamptz default null
)
returns timestamptz
language plpgsql
security invoker
...
begin
  if p_expect is not null then
    target_id := (p_recipes -> 0) ->> 'id';
    select updated_at into found_at from recipes where id = target_id;
    -- Null means the row is new to this account, which is not a conflict.
    if found_at is not null and found_at <> p_expect then
      -- PT409: PostgREST reads a PT-prefixed code as the HTTP status to
      -- answer with, so this arrives as a plain 409 Conflict. It was 40001,
      -- "serialization failure", which the connection pooler treats as a
      -- transient fault and retries — so a refusal took a minute to arrive
      -- and then arrived as a timeout.
      raise exception 'stale_recipe' using errcode = 'PT409';
    end if;
  end if;
  ...
  -- What the screen should hold as its version from now on. Null for a bulk
  -- write, which has no single row to speak for.
  select updated_at into wrote_at from recipes
  where id = (p_recipes -> 0) ->> 'id';
  return wrote_at;
end
$$;

revoke all on function save_recipes(jsonb, jsonb, boolean, timestamptz) from public;
grant execute on function save_recipes(jsonb, jsonb, boolean, timestamptz) to authenticated;

-- The three-argument version from migration 19, now that the four-argument one
-- exists. Left behind it would be the one PostgREST resolved for the old call.
drop function if exists save_recipes(jsonb, jsonb, boolean);
